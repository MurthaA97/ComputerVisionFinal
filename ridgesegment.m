...
function [normim, mask, maskind] = ridgesegment(im, blksze, thresh)
    ...
    % define anonymous function to set the block to the stddev of the 
    % local block
    fun = @(x) std(x.data(:))*ones(size(x.data));
    stddevim = blockproc(im, [blksze blksze], fun);
    
    % if the block stddev is above the threshold then we consider it part
    % of the finger print
